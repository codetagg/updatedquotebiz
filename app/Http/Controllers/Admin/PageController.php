...
class PageController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            return redirect()->action('Admin\PageController@update', array('list_uid' => $list->uid, 'alias' => $layout->alias));
        }
    ...
    public function preview(Request $request)
    {
        ...
        return view('admin.pages.preview_'.$page->layout->type, [
            'list' => $list,
            'page' => $page,
        ]);
    }
    ...
    public function signUpForm(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if ($request->redirect_url) {
                ...
                return redirect()->action('Admin\PageController@signUpThankyouPage', ['list_uid' => $list->uid, 'subscriber_uid' => $subscriber->uid]);
            } else {
                // All done, confirmed
                return redirect()->action(
                    'Admin\PageController@signUpConfirmationThankyou',
                    [
                        'list_uid' => $list->uid,
                        'uid' => $subscriber->uid,
                        'code' => 'empty',
    ...
    public function unsubscribeForm(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            return redirect()->action('Admin\PageController@unsubscribeSuccessPage', ['list_uid' => $list->uid, 'uid' => $subscriber->uid]);
        }
    ...
    public function profileUpdateForm(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            if ($subscriber->getSecurityToken('update-profile') == $request->code) {
                ...
                return redirect()->action('Admin\PageController@profileUpdateSuccessPage', ['list_uid' => $list->uid, 'uid' => $subscriber->uid]);
            }
        }
